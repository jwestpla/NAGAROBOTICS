...
def Oppretning_hoyre():
    ...
    for i in range(iterations):
      motor_serial.send_command(TURNING_SPEED * direction, -TURNING_SPEED * 0)
      time.sleep(0.10)
...
def Oppretning_venstre():
    ...
    for i in range(iterations):
      motor_serial.send_command(TURNING_SPEED * 0, -TURNING_SPEED * direction)
      time.sleep(0.10)
...
while not motor_serial.shutdown_now :
  ...
  elif dist_1 > STOP_DISTANCE and dist_2 > dist_4 and abs(dist_2 - dist_4) > 3 and not abs(dist_2 - dist_4) > 40:
    time.sleep(0.5)
    Oppretning_venstre()
